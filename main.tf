...
resource "azurerm_kubernetes_cluster" "aks002cil" {
  name                 = "aks002cil"
  location             = "westus2"
  resource_group_name  = "prod-clu-grp01"
  dns_prefix           = "aks002cil-k8s"
  azure_policy_enabled = true
...
resource "azurerm_kubernetes_cluster_extension" "aks002cil-extn" {
  name           = "aks002cil-extn"
  cluster_id     = azurerm_kubernetes_cluster.aks002cil.id
  extension_type = "microsoft.flux"
...
resource "azurerm_kubernetes_flux_configuration" "appteam2-aks002cil" {
  name       = "appteam2-aks002cil"
  cluster_id = azurerm_kubernetes_cluster.aks002cil.id
  namespace  = "flux"
  scope      = "cluster"
  ...
  depends_on = [
    azurerm_kubernetes_cluster_extension.aks002cil-extn
  ]
}


resource "azurerm_role_assignment" "regrole-aks002cil" {
  principal_id                     = azurerm_kubernetes_cluster.aks002cil.kubelet_identity[0].object_id
  role_definition_name             = "AcrPull"
  scope                            = "/subscriptions/e049fcf1-c84b-4de4-ba9a-a168a4cbab7a/resourceGroups/acrgrp/providers/Microsoft.ContainerRegistry/registries/dansregwu3"
  skip_service_principal_aad_check = true

  depends_on = [
    azurerm_kubernetes_cluster.aks002cil
