...
### AI team 002 ### 
resource "azurerm_kubernetes_cluster" "app2" {
  name                 = "app2"
  location             = "westus3"
  resource_group_name  = "prod-clu-grp01"
  dns_prefix           = "app2-k8s"
...
resource "azurerm_kubernetes_cluster_extension" "app2-extn" {
  name           = "app2-extn"
  cluster_id     = azurerm_kubernetes_cluster.app2.id
  extension_type = "microsoft.flux"
...
resource "azurerm_kubernetes_flux_configuration" "appteam2-app2" {
  ...
  depends_on = [
    azurerm_kubernetes_cluster_extension.app2-extn
  ]
}


resource "azurerm_role_assignment" "regrole-app2" {
  principal_id                     = azurerm_kubernetes_cluster.app2.kubelet_identity[0].object_id
  role_definition_name             = "AcrPull-app2"
  scope                            = "/subscriptions/e049fcf1-c84b-4de4-ba9a-a168a4cbab7a/resourceGroups/acrgrp/providers/Microsoft.ContainerRegistry/registries/dansregwu3"
  skip_service_principal_aad_check = true

  depends_on = [
    azurerm_kubernetes_cluster.app2
